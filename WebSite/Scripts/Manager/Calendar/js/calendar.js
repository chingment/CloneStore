            ...
            if($('#btn_SetHolidays').length>0){
                $("#GD" + i).unbind('click').click(function(){mOck(this,sD + 1);});
						...
						if (cld[sD].isToday) {
						    ...
						    if(isflag)
						    {
						        $("#GD" + i).addClass("jinri-feijiaoyiri");
						    }
						    else
						    {
						        $("#GD" + i).addClass("jinri-jiaoyiri");
						    }
...
function clear() {
    for (i = 0; i < 42; i++) {
        ...
        $("#GD" + i).removeClass("jinri-jiaoyiri");
        $("#GD" + i).removeClass("jinri-feijiaoyiri");
...
function mOck(thisObj, v){
	...
	var endTime=nian+"/"+addZ(yue)+"/"+addZ(day);

	var isFlag=true;
	if(typeof startDate!="undefined")
	    isFlag=checkStartTimeGreaterThanEndTime(startDate,endTime);

	if(isFlag){
	    if($(thisObj).attr("class")!="unover")
	    {
	        if(onoff == '0'){
	            //thisObj.style.background='#FBBB67';
	            $(thisObj).attr("class","selday");
...
function checkStartTimeGreaterThanEndTime(startTime,endTime){
