...
(function ($) {
    $.lumos = lumos = {
        ...
        post: function (opts) {
            ...
            var uniqueSubmit = $("input[name=_UniqueSubmitIdentifier]").val();
            var l_Data = null;
            if (typeof opts.data == "string") {
                ...
                if (uniqueSubmit != '') {
                    l_Data += "&_UniqueSubmitIdentifier=" + uniqueSubmit;
                }
            }
            else {
                l_Data = [];
            ...
            $.ajax({
                ...
                beforeSend: function (XMLHttpRequest) {
                    $.lumos.common.loadingShow("Loading...");
                },
                ...
                if (!lumos.isPageError(data)) {
                    if (data.result == "Exception") {
                        $("input[name=_UniqueSubmitIdentifier]").val(lumos.newGuid());
                    }
                    else {
                        opts.success(data);
                    }
    ...
    $.fn.operateData = function (opts) {
        ...
        if (opts.multiSelect) {
            ...
            selTexts = "Selected data【" + $(l_DataTable).find("thead tr th").eq(headerTitleIndex).text().trim() + "】 is:<br/>" + selTexts;
        }
        else {
            if (opts.data != null) {
                sekKeys = "" + opts.data + "";
            ...
            if (opts.isUseEmptyRow) {
                ...
                if (p_table.Rows.length < opts.pageSize) {
                    ...
                    for (var i = 0; i < opts.pageSize - p_table.Rows.length; i++) {
                        l_StrEmptyRow += "<tr null=\"true\">"
                        for (var j = 0; j < l_StrEmptyTdCount; j++) {
                            l_StrEmptyRow += "<td></td>"
                        }
                        l_StrEmptyRow += "</tr>"
                    }
                ...
                success: function (data) {
                    ...
                    if (!$.lumos.common.isPageError(data)) {
                        ...
                        if (typeof data.message != undefined) {
                            opts.messageHandleFun(data.message)
                        }
                    }
                }
            });
    ...
    $.fn.tabToTree = function (opts) {
        opts = $.extend({
            tabid: '', //tabId
            selTabFun: function () { return false; }, //
            noSelectTip: "Please select a node on the left！", //当没有在选择
            tabIsSelFirBox: false           //是否选择第一个tabBox
        }, opts || {});
