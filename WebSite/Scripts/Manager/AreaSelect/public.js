...
(function() {
	...
	function() {
		...
		if (currentClass == "proCityQueryAll") {
			...
			if ($("#dimCityQuery").data("inputId") != null) {
			    var selInputId = $("#dimCityQuery").data("inputId");
			    $("#" + selInputId + "Province").val(provinceId);
			    $("#" + selInputId + "City").val(cityId);
			    $("#" + selInputId + "County").val(countyId);
			}
...
function sendAllProvinceAjax() {
	$.ajax({
		type: "get",
		url: '/Scripts/AreaSelect/queryAllProvinces.js',
		async: false,
		dataType: "json",
		success: function(data) {
...
function sendAllCitiesAjax() {
	$.ajax({
		type: "get",
		url: '/Scripts/AreaSelect/queryCities.js',
		async: false,
		dataType: "json",
		success: function (data) {
...
function sendAllCountiesAjax()
 {
	$.ajax({
		type: "get",
		url: "/Scripts/AreaSelect/queryAllAreas.js",
		async: false,
		dataType: "json",
		success: function(data) {
			allAreas = data.areas;
...
function viewAllHotCities() {
	...
	function(i, city) {
	    if (city.hotCity) {
	        ...
	        else if (cityname == "天津市辖") {
	            cityname = "天津";
	        }
	        else if (cityname == "上海市辖") {
	            cityname = "上海";
	        }
	        else if (cityname == "香港特区") {
	            cityname = "香港";
	        }
	        else if (cityname == "澳门特区") {
	            cityname = "澳门";
	        }
	        $(".hotCityAll .list ul").append("<li><a><input type='button' style='background:none;border:0px;cursor: pointer;' onclick=hotCityAddrInputAll(\'" + city.provinceId + "," + city.id + "," + city.name + "\') id='" + city.id + "' value='" + cityname + "'></a></li>");
		}
	});
}
...
function viewAllProvince(page) {
	...
	for (var i = start; i < end; i++) {
		...
		if (allProvinces[i].provinceName == '内蒙古') {
			p_name = '内蒙古';
...
function viewAllCities(i) {
	...
	if ($("#dimCityQuery").data("inputId") != null) {
	    var selInputId = $("#dimCityQuery").data("inputId");
	    $("#" + selInputId + "Province").val(allProId);
	}
...
function getSelectAreaText(provinceId, cityId, countyId) {
    ...
    $.each(allProvinces, function (idx, obj) {
        if (obj.id == ProvinceId) {
            provinceName = obj.provinceName;
        }
    });

    $.each(allCities, function (idx, obj) {
        if (obj.id == cityId) {
            cityName = obj.name;
        }
    });

    $.each(allAreas, function (idx, obj) {
        if (obj.id == countyId) {
            countyName = obj.areaName;
        }
    });
    var s = provinceName;
    if (cityName != "")
