    ...
    $.formValidator =
{
    ...
    showMessage: function (returnObj) {
        ...
        if (!isValid) {
            ...
            if (empty) {
                ...
                //alert(returnObj.valLen)
                if (returnObj.valLen > 0) {
                    $.formValidator.setTipState(elem, "onCorrect", showmsg);
                }
                else {

                    $.formValidator.setTipState(elem, "none", showmsg);
                }
            }
            else {
                $.formValidator.setTipState(elem, "onCorrect", showmsg);
    ...
    pageIsValid: function (validatorGroup) {
        ...
        if (isValid) {
            ...
            if (theFirstid.indexOf('DateTime') > -1) {
            }
            else {
        ...
        // alert(!initConfig.debug && isValid)
    ...
    setformTip: function (msg, error) {


        if (typeof error == "undefined") {
            $(".form-tip").html("<p>" + msg + "</p>");
        }
        else {
            var html = "";
    ...
    //指定控件显示通过或不通过样式
    $.fn.defaultPassed = function (onShow, tip) {
     
        var tagName = this.attr("tagName");
        if (tagName == "FORM") {
            var o = $(this).find("select, input[type = text], input[type = password], input[type = radio], input[type = checkbox]");
                    ...
                    for (var i = 1; i < settings.length; i++) {
                        ...
                        if (!$('body').data(settings[0].validatorGroup).alertMessage) {
                            if (onShow != "none") {
                                ...
                                if (ls_style == "onShow") {
                                    $.formValidator.setTipState(this, ls_style, settings[0].onShow);
                                }
                                else {
                ...
                if (typeof this.settings != "undefined") {
                    ...
                    for (var i = 1; i < settings.length; i++) {
                        ...
                        if (!$('body').data(settings[0].validatorGroup).alertMessage) {
                            ...
                            if (onShow == "none") {
                                $.formValidator.setTipState(this, "", null);
                            }
                            else {
                          
                                if (typeof onShow == "undefined") {
                                    $.formValidator.setTipState(this, "onCorrect", settings[0].onCorrect);
                                }
                                else {
                                    $.formValidator.setTipState(this, onShow, tip);
                                }
                            }
